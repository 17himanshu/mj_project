...
authRouter.post("/login", async (req, res) => {
  ...
  try {
    ...
    // Get user data from user_login view for additional info
    const userDataQuery = "select * from user_login where id = $1";
    const userData = await pool.query(userDataQuery, [result.id]);
    ...
    return res.json({
      ...
      data: {
        ...
        fullName: userData.rows[0].full_name,
        email: userData.rows[0].email,
        idNumber: userData.rows[0].id_number,
        phone: userData.rows[0].phone,
        dateOfbirth: userData.rows[0].date_of_birth,
        image_name: userData.rows[0].image_name,
        image_path: userData.rows[0].profile_image_path,
        sitter_id: userData.rows[0].pet_sitter_id,
        sitter_image_name: userData.rows[0].pet_sitter_image_name,
        sitter_image_path: userData.rows[0].pet_sitter_profile_image_path,
        sitter_authen: userData.rows[0].sitter_authen,
      },
    });
  } catch (err) {
